...
class AudioAPI(object):
    ...
    for i in range(0,len(beg)):
        if((end[i]-beg[i])>110):
            time = self.gettime(beg,winstep,fs)
            timestamp =  (floor(time*10))/10
            events.append(create_event(event_type=:"Tag",
                        event_subtype="Loud",
                        timestamp=timestamp))
    ...
    tags, watches, fus = self.check_responses(responses,buff_beg)
    tags = self.debounce(tags)
    watches = self.debounce(watches)
    fus = self.debounce(fus)
    for i in tags:
        time = self.gettime(i,winstep,fs)
            timestamp =  (floor(time*10))/10
            events.append(self.create_event(event_type=:"Tag",
                        event_subtype="Tag",
                        timestamp=timestamp))
    ...
    time = float(win*winstep)
    return (time/fs)

    def getsamp(self,win,winsize,fs):
